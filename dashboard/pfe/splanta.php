...
$sql = "select * from plant as a,client as b where b.ID=a.id_clt and a.valid=1 order by a.date DESC";
...
array('id'=>$row[0],
'sname'=>$row[1],
'cname'=>$row[2],'family'=>$row[3],'desc'=>$row[4],'location'=>$row[5],'Utilisation'=>$row[6],'img_path'=>$row[7],'valid'=>$row[8],'date'=>$row[9],'id_clt'=>$row[10],'email'=>$row[12]
));
echo $result;
}
